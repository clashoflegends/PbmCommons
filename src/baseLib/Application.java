...
/**
 * Base Application class for Swing apps. Application provides the following functionality: <ul> <li>A sequence of methods to override that are invoked as part
 * of starting the application. All methods are invoked on the event dispatch thread. <li>Has a ResourceBundle. This is useful for smallish apps that wish to
 * place all their localized resources into a single place. <li>Has a Preferences node. This is useful for apps that wish to persist state between sessions.
 * <li>Registers itself as the UncaughtExceptionHandler. If an uncaught exception is encountered the application will show a warning dialog and exit.
 * <li>Provides a single entry point to call when the user wishes to exit the app. This can be overriden to warn the user about unsaved changes, or provide
 * similar functionality. <li>Provides a ThreadFactory that produces background threads suitable for Swing applications. The application will block until all
 * background threads vended from the thread factory have completed. <li>An arbitrary set of key/value pairs. </ul>
 * <p>
 * <a
 * name="initSequence"></a> Application provides a handful of methods that are invoked as part of starting the application. Subclasses need only override those
 * they are interested in. The following outlines the order the methods are invoked in as well as what they are intended for. All methods are invoked on the
 * event dispatching thread. <ol> <li>preInit is invoked first. preInit is provided for any initialization that needs to be done prior to creating the UI.
 * Application's implementation invokes installLookAndFeel to install the system look and feel. <li>init is invoked after preInit. init is intended for creating
 * the actual UI. <li>ApplicationListeners are notified that the application has initialized (appDidInit). <li>postInit is invoked after the listeners are
 * notified, and is intended for any cleanup that needs to be done.
 * <li>postInitEventQueueEmpty is invoked after postInit and after the event queue has processed all pending events, such as paint events or revalidate requests
 * generated during the earlier stages of initialization. Subclasses that need to do processing after the UI is completely showing should override this method.
 * </ol>
 * <p>
 * <a name="exitSequence"> Application also provides a sequence of methods for exiting the application. When exit is invoked the following methods are invoked:
 * <ol> <li>canExit is invoked, if this returns false the application will not exit. <li>ApplicationListeners are asked if the application can exit. If any
 * IApplicationListener returns false from canApplicationExit the application will not exit.
 * <li>waitForBackgroundThreads is invoked to block until any background threads have completed. <li>All ApplicationListeners are notified that the application
 * is exiting. <li>exiting is invoked. <li>Lastly, System.exit is invoked. </ol>
 * <p>
...
public abstract class Application implements Thread.UncaughtExceptionHandler, Serializable {
    ...
    /**
     * Creates a new Application instance. Subclasses very rarely need put any logic in the constructor, especially not code that creates Swing components.
     * Override one of the various init methods instead which are guaranteed to be invoked on the event dispatching thread.
     *
    ...
    /**
     * Returns the key for loading the resources for the Application. This implementation returns <code>getClass().getName().resources.Resources</code>.
     *
    ...
    /**
     * Returns a ThreadFactory suitable for threads used within Swing applications. Threads created by the returned ThreadFactory are automatically registered
     * with the Application, are not daemon, and have a priority of Thread.MIN_PRIORITY.
     * <p>
    ...
    /**
     * Invoked from preInit to set the look and feel for the Application. This implementation sets the look and feel to the system look and feel. If setting the
     * look and feel results in throwing an exception it will be ignored.
     *
     * @see <a href="#initSequence">init sequence</a>
     */
...
//            JFrame.setDefaultLookAndFeelDecorated(false);
    ...
    /**
     * Invoked as part of starting the application. This method invokes installLookAndFeel. See <a href="#initSequence">init sequence</a> for details on when
     * this is invoked.
     *
    ...
    /**
     * Blocks until all registered threads have completed. This is invoked from exit. If necessary this method will invoke createBackgroundThreadDialog to
     * create a modal dialog that is shown while waiting for background threads to exit.
     *
     * @see #exit
    ...
    protected JDialog getUncaughtExceptionDialog() {
        ...
        if (this.throwable instanceof java.lang.OutOfMemoryError) {
            errorMsg = String.format("Java ran out of memory. %s will now exit. Just relaunch it and it will be fine.", getName());
        } else {
            errorMsg = String.format("An unrecoverable error has occured. %s will now exit. You can send the Counselor.log file to the GM for investigation.", getName());
        }
